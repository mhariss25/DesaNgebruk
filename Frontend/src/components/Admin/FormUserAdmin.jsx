...
const FormUserAdmin = () => {
  ...
  useEffect(() => {
    ...
    if (id_user) {
      axios
        ...
        .then((response) => {
          const { nama, username, email, role } = response.data;
          setNama(nama);
  ...
  const handleSubmit = async (e) => {
    ...
    const userData = { nama, username, email, role };
    ...
    try {
      ...
      navigate("/blogger/all_user");
    } catch (error) {
      console.error("Error:", error);
      Swal.fire(
        "Error",
        `Gagal ${id_user ? "memperbarui" : "menambahkan"} user`,
        "error"
      );
    }
    setNama("");
    ...
    <section className="p-4 mt-20 bg-white dark:bg-gray-900">
      <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        ...
        <form onSubmit={handleSubmit}>
          ...
          >
            {id_user ? "Update User" : "Tambah User"}
          </button>
        </form>
      </div>
    </section>
